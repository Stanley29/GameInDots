...
function fillButton0(){
  ...
  document.getElementById(currentBtnId).style.backgroundColor = "blue";
  //
...
//function for in-game buttons processing
  function reply_click(clicked_id)
  {
    if(play){
      //alert(clicked_id);      
      if(currentBtnId == clicked_id){
        document.getElementById(currentBtnId).style.backgroundColor = "green";
...
$.get('https://starnavi-frontend-test-task.herokuapp.com/winners',function(data){
  //console.log(data); 
  var k;
  for(k in data)
  {
    //console.log(data[k].winner);
    //console.log(data[k].date);    
  }

  var table = document.getElementById('winner_board');
  data.forEach(function(object) {
    var tr = document.createElement('tr');
    tr.innerHTML = '<td>' + object.winner + '</td>' +
        '<td>' + object.date + '</td>';
